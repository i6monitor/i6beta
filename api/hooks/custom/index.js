      ...
      before: {
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            var loggedInUser = await Si6_user.findOne({
            ...
            if (loggedInUser.lastSeenAt < now - MS_TO_BUFFER) {
              Si6_user.update({id: loggedInUser.id})
              .set({ lastSeenAt: now })
              .exec((err)=>{
                if (err) {
            ...
            if (req.method === 'GET') {
              ...
              for (let attrName in Si6_user.attributes) {
                if (Si6_user.attributes[attrName].protect) {
